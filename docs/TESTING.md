...
依赖：JDK 8+、Maven。测试使用 H2 内存库（见 `backend/src/test/resources/application.properties`），无需 Neon / `DATABASE_URL`。
...
| 自动分类关键词规则 | `TicketServiceClassifyTest` | 单测包可见 `classify` |
...
- **自动分类**：规则测在 `TicketService.classify`；创建接口测验证 Controller 正确返回服务层给出的 `category`。
...
3. **自动分类非端到端**：分类规则与 Controller 创建用例分离；未验证「请求关键词 → DB 落库 category」全链路。
